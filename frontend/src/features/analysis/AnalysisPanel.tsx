...
export const AnalysisPanel = ({ onClose, repositoryId }: AnalysisPanelProps) => {
 ...
 if (loading) {
  return (
   <div className="h-full flex flex-col glass-card border border-zinc-400/10 rounded-2xl shadow-sm overflow-hidden items-center justify-center">
 ...
 if (!data) {
   return (
   <div className="h-full flex flex-col glass-card border border-zinc-400/10 rounded-2xl shadow-sm overflow-hidden items-center justify-center p-8 text-center">
 ...
 const tabs = [
  ...
  { id: 'coupling', label: 'High Coupling', icon: Cpu, count: data.top_coupled_files?.length || 0, color: 'text-[var(--accent)]', bg: 'bg-[var(--accent-soft)]' },
 ...
 const panelContent = isExpanded ? (
  <motion.div 
   ...
   className="w-full h-full max-w-6xl max-h-[85vh] flex flex-col bg-[#0B0B0F] border border-zinc-400/10 rounded-2xl shadow-sm overflow-hidden mx-auto text-zinc-200"
  >
   {/* Expanded Header */}
  <div className="flex items-center justify-between p-6 border-b border-zinc-400/10 bg-transparent shrink-0">
    <div className="flex items-center gap-4">
    <div className="w-12 h-12 rounded-xl bg-zinc-950 flex items-center justify-center shadow-sm">
      <Cpu className="w-6 h-6 text-zinc-300" />
     </div>
     <div>
    <h2 className="text-xl font-bold text-zinc-200 tracking-tight">Codebase Intelligence</h2>
    ...
    <div className="flex items-center gap-2">
     <button
      ...
      className="p-2.5 rounded-xl text-zinc-400 hover:text-zinc-200 hover:bg-zinc-950/10 transition-colors"
      title="Minimize"
     >
  ...
  <div className="flex flex-1 overflow-hidden">
    {/* Sidebar */}
  <div className="w-72 border-r border-zinc-400/10 bg-black/20 p-4 space-y-2 overflow-y-auto shrink-0 custom-scrollbar">
      {tabs.map(tab => (
       <button
        key={tab.id}
        ...
        className={`w-full flex items-center gap-3 px-4 py-3.5 rounded-xl text-left transition-all duration-200 ${
         ...
         : 'text-zinc-400 hover:bg-zinc-950/10 hover:text-zinc-200 border border-transparent'
        }`}
       >
        <tab.icon className={`w-5 h-5 ${activeTabId === tab.id ? 'text-zinc-200' : tab.color}`} />
        <span className="text-sm font-medium flex-1">{tab.label}</span>
        <span className={`px-2.5 py-0.5 rounded-full text-xs font-bold ${activeTabId === tab.id ? 'bg-zinc-800 text-zinc-300 border border-zinc-700' : 'bg-black/50 text-zinc-200'}`}>
         {tab.count}
        </span>
       </button>
      ))}
    ...
    {/* Main Area */}
    <div className="flex-1 overflow-y-auto p-8 bg-black custom-scrollbar text-zinc-200">
     <div className="flex items-center gap-4 mb-8 pb-4 border-b border-zinc-400/5">
       <div className={`p-3 rounded-xl ${activeTab.bg}`}>
        <activeTab.icon className={`w-6 h-6 ${activeTab.color}`} />
       </div>
      <h3 className="text-2xl font-bold text-zinc-200 tracking-tight">{activeTab.label}</h3>
     </div>
     ...
     {activeTabId === 'god_classes' && (
       <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
        {data.top_giant_files?.map((cls: any, idx: number) => (
          <div key={idx} className="bg-zinc-950/5 border border-zinc-400/10 p-5 rounded-2xl flex flex-col justify-between hover:border-rose-500/40 transition-colors shadow-sm">
          <p className="text-sm font-mono text-zinc-200 break-all mb-5" title={cls.name}>{cls.name}</p>
          <div className="flex items-center gap-6 mt-auto">
           ...
           <div className="w-px h-8 bg-zinc-950/10" />
           <div>
     ...
     {activeTabId === 'circular_deps' && (
       <div className="space-y-4">
        {data.circular_dependencies?.map((cycle: string[], idx: number) => (
         <div key={idx} className="bg-zinc-950/5 border border-zinc-400/10 p-6 rounded-2xl hover:border-amber-500/40 transition-colors shadow-sm">
          ...
          <div className="flex flex-col gap-3 relative pl-3">
           ...
           {cycle.map((file, fileIdx) => (
            <div key={fileIdx} className="flex items-center gap-4 relative z-10">
             <div className="w-2.5 h-2.5 rounded-full bg-amber-500 ring-4 ring-zinc-400" />
             <span className="text-sm font-mono text-zinc-200 break-all bg-black/20 px-3 py-1.5 rounded-lg border border-zinc-400/5">
              {file}
             </span>
            </div>
           ))}
     ...
     {activeTabId === 'coupling' && (
        ...
        {data.top_coupled_files?.map((file: any, idx: number) => (
          <div key={idx} className="bg-zinc-950/5 border border-zinc-400/10 p-5 rounded-2xl flex flex-col justify-between hover:border-zinc-400/10 transition-colors shadow-sm">
            <p className="text-sm font-mono text-zinc-200 break-all mb-5" title={file.name}>{file.name}</p>
          <div className="flex items-center gap-6 mt-auto">
           <div>
     ...
     {activeTabId === 'hotspots' && (
       <div className="grid grid-cols-1 gap-3">
         {data.architectural_hotspots?.map((file: any, idx: number) => (
          <div key={idx} className="bg-zinc-950/5 border border-zinc-400/10 p-5 rounded-2xl flex items-center justify-between hover:border-fuchsia-500/40 transition-colors shadow-sm">
              ...
              <div className="text-right shrink-0 ml-4 bg-black/20 px-4 py-2 rounded-xl border border-zinc-400/5">
     ...
     {activeTabId === 'dead_code' && (
       <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
         {data.dead_code_candidates?.map((file: any, idx: number) => (
            <div key={idx} className="bg-zinc-950/5 border border-zinc-400/10 p-5 rounded-2xl flex items-center gap-4 hover:border-zinc-500/40 transition-colors shadow-sm group">
  ...
  <motion.div 
   layoutId="analysis-panel"
   className="h-full flex flex-col soft-card border border-zinc-400/10 rounded-2xl shadow-sm overflow-hidden bg-[#0B0B0F] text-zinc-200"
  >
   {/* Mini Header */}
  <div className="flex items-center justify-between p-5 border-b border-zinc-400/10 bg-transparent shrink-0">
    <div className="flex items-center gap-3">
    <div className="w-8 h-8 rounded-md bg-zinc-950 flex items-center justify-center">
     <Cpu className="w-4 h-4 text-zinc-200" />
     </div>
     <div>
     <h2 className="text-base font-semibold text-zinc-200 tracking-tight">Codebase Intelligence</h2>
     <p className="text-[11px] text-zinc-400 font-medium">Real-time architecture metrics</p>
     </div>
    </div>
    <div className="flex items-center gap-1.5">
     <button
      onClick={() => handleExpand(activeTabId || 'god_classes')}
      className="p-1.5 rounded-lg text-zinc-400 hover:text-zinc-200 hover:bg-zinc-950/10 transition-colors"
  ...
  <div className="flex-1 overflow-y-auto p-5 space-y-6 custom-scrollbar bg-black/20">
    ...
    <section className="space-y-3">
     <div className="flex items-center gap-2 px-1">
      ...
      <button onClick={() => handleExpand('god_classes')} className="p-1 rounded-md text-zinc-400 hover:text-zinc-200 hover:bg-zinc-950/10 transition-colors ml-1" title="Expand God Classes">
       <Maximize2 className="w-3.5 h-3.5" />
      </button>
     </div>
     <div className="flex flex-col gap-2">
      {data.top_giant_files && data.top_giant_files.map((cls: any, idx: number) => (
       <div key={idx} className="bg-zinc-950/5 border border-zinc-400/10 p-3 rounded-xl hover:border-rose-500/30 transition-colors group">
        <p className="text-xs font-mono text-zinc-200 truncate mb-2" title={cls.name}>{cls.name.split('/').pop()}</p>
        <div className="flex items-center gap-4">
         <div>
         ...
         <div className="w-px h-6 bg-zinc-200/10" />
         <div>
    ...
    <div className="h-px w-full bg-zinc-200/5" />
    ...
    <section className="space-y-3">
     <div className="flex items-center gap-2 px-1">
      ...
      <button onClick={() => handleExpand('circular_deps')} className="p-1 rounded-md text-zinc-400 hover:text-zinc-200 hover:bg-zinc-950/10 transition-colors ml-1" title="Expand Circular Dependencies">
       <Maximize2 className="w-3.5 h-3.5" />
      </button>
     </div>
     <div className="flex flex-col gap-2">
      {data.circular_dependencies && data.circular_dependencies.map((cycle: string[], idx: number) => (
      <div key={idx} className="bg-zinc-950/5 border border-zinc-400/10 p-3 rounded-xl hover:border-amber-500/30 transition-colors">
        <div className="flex flex-col gap-1.5">
         {cycle.map((file, fileIdx) => (
          <div key={fileIdx} className="flex items-center gap-2">
           <div className="w-1.5 h-1.5 rounded-full bg-amber-500/50" />
            <span className="text-[11px] font-mono text-zinc-200 truncate" title={file}>
            {file.split('/').pop()}
           </span>
          </div>
         ))}
    ...
    <section className="space-y-3">
     <div className="flex items-center gap-2 px-1">
      ...
      <span className="ml-auto bg-zinc-800 text-zinc-200 text-[10px] font-bold px-2 py-0.5 rounded-full">{data.top_coupled_files?.length || 0}</span>
      <button onClick={() => handleExpand('coupling')} className="p-1 rounded-md text-zinc-400 hover:text-zinc-200 hover:bg-zinc-950/10 transition-colors ml-1" title="Expand Coupling">
       <Maximize2 className="w-3.5 h-3.5" />
      </button>
     </div>
     <div className="grid grid-cols-2 gap-2">
      {data.top_coupled_files && data.top_coupled_files.map((file: any, idx: number) => {
       ...
       return (
        <div key={idx} className="bg-zinc-950/5 border border-zinc-400/10 p-3 rounded-xl flex flex-col justify-between hover:border-zinc-400/10 transition-colors group">
         <span className="text-[11px] font-mono text-zinc-200 truncate mb-3" title={file.name}>{fileName}</span>
    ...
    <section className="space-y-3">
     <div className="flex items-center gap-2 px-1">
      ...
      <button onClick={() => handleExpand('hotspots')} className="p-1 rounded-md text-zinc-400 hover:text-zinc-200 hover:bg-zinc-950/10 transition-colors ml-1" title="Expand Hotspots">
       <Maximize2 className="w-3.5 h-3.5" />
      </button>
     </div>
     <div className="flex flex-col gap-2">
      {data.architectural_hotspots && data.architectural_hotspots.slice(0, 5).map((file: any, idx: number) => (
        <div key={idx} className="bg-zinc-950/5 border border-zinc-400/10 p-3 rounded-xl flex items-center justify-between hover:border-fuchsia-500/30 transition-colors group">
         <span className="text-xs font-mono text-zinc-200 truncate max-w-[150px]" title={file.name}>{file.name.split('/').pop()}</span>
    ...
    <section className="space-y-3 pb-4">
     <div className="flex items-center gap-2 px-1">
      ...
      <button onClick={() => handleExpand('dead_code')} className="p-1 rounded-md text-zinc-400 hover:text-zinc-200 hover:bg-zinc-950/10 transition-colors ml-1" title="Expand Dead Code">
       <Maximize2 className="w-3.5 h-3.5" />
      </button>
     </div>
     <div className="flex flex-col gap-2">
       {data.dead_code_candidates && data.dead_code_candidates.slice(0, 5).map((file: any, idx: number) => (
        <div key={idx} className="bg-zinc-950/5 border border-zinc-400/10 p-3 rounded-xl flex items-center gap-2 hover:border-zinc-500/30 transition-colors group">
          <Trash2 className="w-3.5 h-3.5 text-zinc-500 group-hover:text-rose-400 transition-colors shrink-0" />
          <span className="text-xs font-mono text-zinc-200 truncate" title={file.name}>{file.name.split('/').pop()}</span>
        </div>
       ))}
     </div>
    </section>
 ...
 return (
  <>
   {!isExpanded ? (
    ...
    <div className="h-full flex flex-col items-center justify-center p-6 text-center border border-zinc-400/10 rounded-2xl bg-transparent shadow-sm">
     <Cpu className="w-8 h-8 text-zinc-500 mb-3" />
     <p className="text-sm text-zinc-400">Analysis is opened in full screen</p>
     <button onClick={() => setIsExpanded(false)} className="mt-4 px-4 py-2 bg-zinc-950/5 hover:bg-zinc-950/10 text-xs text-zinc-200 rounded-lg transition-colors">
      Close Full Screen
     </button>
    </div>
   )}
    ...
    <AnimatePresence>
     {isExpanded && (
      <motion.div 
       ...
       className="fixed top-14 left-0 md:left-64 right-0 bottom-0 z-[100] flex items-center justify-center p-4 md:p-8 bg-black/40 backdrop-blur-sm pointer-events-auto"
      >
       {panelContent}
      </motion.div>
     )}
