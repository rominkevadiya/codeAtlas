...
export const AnalysisPanel = ({ onClose, repositoryId }: AnalysisPanelProps) => {
 ...
 const tabs = [
  ...
  { id: 'coupling', label: 'High Coupling', icon: Cpu, count: data.top_coupled_files?.length || 0, color: 'text-white', bg: 'bg-zinc-950/10' },
   ...
   <div className="flex items-center justify-between p-6 border-b border-white/10 bg-transparent shrink-0">
    ...
    <div className="flex items-center gap-2">
     <button
      ...
      className="p-2.5 rounded-xl text-zinc-400 hover:text-white hover:bg-zinc-950/10 transition-colors"
      title="Minimize"
     >
      ...
      {tabs.map(tab => (
       <button
        ...
        className={`w-full flex items-center gap-3 px-4 py-3.5 rounded-xl text-left transition-all duration-200 ${
         ...
         : 'text-zinc-400 hover:bg-zinc-950/5 hover:text-zinc-200 border border-transparent'
        }`}
       >
    ...
    <div className="flex-1 overflow-y-auto p-8 bg-black custom-scrollbar">
     ...
     {activeTabId === 'god_classes' && (
       <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
        {data.top_giant_files?.map((cls: any, idx: number) => (
         <div key={idx} className="bg-zinc-950/5 border border-white/10 p-5 rounded-2xl flex flex-col justify-between hover:border-rose-500/40 transition-colors shadow-sm">
     ...
     {activeTabId === 'circular_deps' && (
       <div className="space-y-4">
        {data.circular_dependencies?.map((cycle: string[], idx: number) => (
         <div key={idx} className="bg-zinc-950/5 border border-white/10 p-6 rounded-2xl hover:border-amber-500/40 transition-colors shadow-sm">
     ...
     {activeTabId === 'coupling' && (
        ...
        {data.top_coupled_files?.map((file: any, idx: number) => (
         <div key={idx} className="bg-zinc-950/5 border border-white/10 p-5 rounded-2xl flex flex-col justify-between hover:border-white/10 transition-colors shadow-sm">
     ...
     {activeTabId === 'hotspots' && (
       <div className="grid grid-cols-1 gap-3">
         {data.architectural_hotspots?.map((file: any, idx: number) => (
           <div key={idx} className="bg-zinc-950/5 border border-white/10 p-5 rounded-2xl flex items-center justify-between hover:border-fuchsia-500/40 transition-colors shadow-sm">
     ...
     {activeTabId === 'dead_code' && (
       <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
         {data.dead_code_candidates?.map((file: any, idx: number) => (
            <div key={idx} className="bg-zinc-950/5 border border-white/10 p-5 rounded-2xl flex items-center gap-4 hover:border-zinc-500/40 transition-colors shadow-sm group">
   ...
   <div className="flex-1 overflow-y-auto p-5 space-y-6 custom-scrollbar">
    ...
    <section className="space-y-3">
     <div className="flex items-center gap-2 px-1">
      ...
      <button onClick={() => handleExpand('god_classes')} className="p-1 rounded-md text-zinc-400 hover:text-white hover:bg-zinc-950/10 transition-colors ml-1" title="Expand God Classes">
      ...
      {data.top_giant_files && data.top_giant_files.map((cls: any, idx: number) => (
       <div key={idx} className="bg-[#111115] border border-white/5 p-3 rounded-xl hover:border-rose-500/30 transition-colors group">
        ...
        <div className="flex items-center gap-4">
         ...
         <div className="w-px h-6 bg-zinc-950/5" />
         <div>
    ...
    <section className="space-y-3">
     <div className="flex items-center gap-2 px-1">
      ...
      <button onClick={() => handleExpand('circular_deps')} className="p-1 rounded-md text-zinc-400 hover:text-white hover:bg-zinc-950/10 transition-colors ml-1" title="Expand Circular Dependencies">
       <Maximize2 className="w-3.5 h-3.5" />
      </button>
     </div>
     <div className="flex flex-col gap-2">
    ...
    <section className="space-y-3">
     <div className="flex items-center gap-2 px-1">
      ...
      <button onClick={() => handleExpand('coupling')} className="p-1 rounded-md text-zinc-400 hover:text-white hover:bg-zinc-950/10 transition-colors ml-1" title="Expand Coupling">
       <Maximize2 className="w-3.5 h-3.5" />
      </button>
     </div>
    ...
    <section className="space-y-3">
     <div className="flex items-center gap-2 px-1">
      ...
      <button onClick={() => handleExpand('hotspots')} className="p-1 rounded-md text-zinc-400 hover:text-white hover:bg-zinc-950/10 transition-colors ml-1" title="Expand Hotspots">
       <Maximize2 className="w-3.5 h-3.5" />
      </button>
     </div>
     <div className="flex flex-col gap-2">
    ...
    <section className="space-y-3 pb-4">
     <div className="flex items-center gap-2 px-1">
      ...
      <button onClick={() => handleExpand('dead_code')} className="p-1 rounded-md text-zinc-400 hover:text-white hover:bg-zinc-950/10 transition-colors ml-1" title="Expand Dead Code">
       <Maximize2 className="w-3.5 h-3.5" />
      </button>
     </div>
     <div className="flex flex-col gap-2">
