...
export const ChatPanel = ({ onClose, repositoryId }: ChatPanelProps) => {
 ...
 const handleSend = async (content?: string) => {
  ...
  const panelContent = (
   <motion.div 
    layoutId="chat-panel"
    className={`flex min-w-0 flex-col overflow-hidden border border-zinc-400/10 bg-[#0B0B0F] shadow-sm text-zinc-200 ${isExpanded ? 'w-full h-full max-w-5xl max-h-[85vh] rounded-2xl mx-auto' : 'h-full rounded-2xl'}`}
   >
    {/* Header */}
  <div className="flex items-center justify-between p-4 border-b border-zinc-400/5 bg-transparent shrink-0">
    <div className="flex items-center gap-3">
    <div className="w-8 h-8 rounded-md bg-zinc-950 flex items-center justify-center shadow-sm">
     <Sparkles className="w-4 h-4 text-zinc-200" />
     </div>
     <div>
    <h2 className="text-sm font-semibold text-zinc-200">CodeAtlas AI</h2>
    ...
    <div className="flex items-center gap-1.5">
     ...
     <button onClick={onClose} className="p-1.5 rounded-lg text-zinc-400 hover:text-zinc-200 hover:bg-zinc-950/10 transition-colors" title="Close">
      <X className="w-4 h-4" />
     </button>
    </div>
   </div>
   ...
   {repositoryId && (
    <>
      ...
      <div className="flex-1 overflow-y-auto custom-scrollbar p-3 space-y-2">
       {isLoadingSessions ? (
        ...
        <div className="flex flex-col items-center justify-center h-full gap-4 py-8 text-center">
         ...
         <div>
          <p className="text-sm font-medium text-zinc-200">No conversations yet</p>
         ...
         <div className="space-y-2 w-full mt-2">
          {STARTER_PROMPTS.map((prompt) => (
           <button
            ...
            className="w-full text-left text-xs text-zinc-200 bg-zinc-950/5 hover:bg-zinc-800 hover:text-zinc-200 border border-zinc-400/5 hover:border-zinc-400/10 rounded-md p-3 transition-all"
           >
            {prompt}
           </button>
          ))}
        ...
        <>
         ...
         {sessions.map(session => (
          <div
           ...
           className="group flex items-start gap-3 p-3 rounded-xl bg-zinc-950/[0.02] hover:bg-zinc-950/[0.05] border border-zinc-400/5 hover:border-zinc-400/10 cursor-pointer transition-all"
          >
           <div className="w-7 h-7 rounded-md bg-zinc-800 flex items-center justify-center shrink-0 mt-0.5">
            <MessageSquare className="w-3.5 h-3.5 text-zinc-200" />
           </div>
         ...
         {STARTER_PROMPTS.map((prompt) => (
          <button
           key={prompt}
           onClick={() => handleSend(prompt)}
           className="w-full text-left text-xs text-zinc-400 hover:text-zinc-200 bg-zinc-950/[0.02] hover:bg-zinc-800 border border-zinc-400/5 hover:border-zinc-400/10 rounded-md p-3 transition-all"
          >
           {prompt}
          </button>
         ))}
     ...
     {activeSession && (
      <>
       ...
       <button
        ...
        className="mx-3 mt-2 text-[10px] text-zinc-500 hover:text-zinc-200 transition-colors text-left px-2"
       >
        ← All conversations
       </button>
      ...
      <div className="flex-1 overflow-y-auto p-4 space-y-5 custom-scrollbar">
        ...
        {messages.map((msg) => (
         <div key={msg.id} className={`flex gap-3 ${msg.role === 'user' ? 'flex-row-reverse' : 'flex-row'}`}>
          <div className={`w-7 h-7 rounded-md shrink-0 flex items-center justify-center ${msg.role === 'user' ? 'bg-zinc-800 text-zinc-200' : 'bg-zinc-800 text-zinc-300 border border-zinc-700'}`}>
           {msg.role === 'user' ? <User className="w-3.5 h-3.5" /> : <Bot className="w-3.5 h-3.5" />}
          </div>
          <div className={`max-w-[85%] rounded-lg p-3.5 text-sm leading-relaxed ${msg.role === 'user' ? 'bg-zinc-800 text-zinc-200 rounded-tr-sm border border-zinc-700' : 'bg-[#0A0A0A] text-zinc-200 rounded-tl-sm border border-zinc-400/10 shadow-sm'}`}>
            ...
            <div className="markdown-body">
             <ReactMarkdown
              components={{
               ...
               h1: ({...props}) => <h1 className="text-2xl font-bold text-zinc-200 mb-4 border-b border-zinc-800 pb-2 mt-6 first:mt-0" {...props} />,
               h2: ({...props}) => <h2 className="text-xl font-semibold text-zinc-200 mb-3 mt-6 pb-2 border-b border-zinc-800/50" {...props} />,
        ...
        {isTyping && (
         <div className="flex gap-3">
          <div className="w-7 h-7 rounded-md shrink-0 flex items-center justify-center bg-[var(--accent)] text-zinc-200">
           <Bot className="w-3.5 h-3.5" />
          </div>
          <div className="rounded-lg rounded-tl-sm p-3.5 bg-[#0A0A0A] border border-zinc-400/10 flex items-center gap-2">
     ...
     {/* Input Area */}
    <div className="p-3 bg-black/20 border-t border-zinc-400/5 shrink-0">
      <form onSubmit={(e) => { e.preventDefault(); handleSend(); }} className="relative flex items-center">
       <input
        ...
        className="w-full bg-[#0A0A0A] border border-zinc-400/10 rounded-md py-3 pl-4 pr-12 text-sm text-zinc-200 placeholder:text-zinc-500 focus:outline-none focus:border-zinc-400/20 transition-all shadow-sm disabled:opacity-50"
       />
       <button
        type="submit"
        disabled={!input.trim() || isTyping}
        className="absolute right-2 p-2 rounded-md bg-zinc-800 border border-zinc-700 hover:bg-zinc-700 text-zinc-300 transition-colors disabled:opacity-50 disabled:cursor-not-allowed"
       >
 ...
 return (
  <>
   {!isExpanded ? (
    ...
    <div className="h-full flex flex-col items-center justify-center p-6 text-center border border-zinc-400/10 rounded-md bg-transparent shadow-sm">
